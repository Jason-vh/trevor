...
async function processQueue(bot: Bot): Promise<void> {
  ...
  try {
    ...
    logger.info(`Scheduler: processing ${entries.length} queue entries`);
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        if (result.success) {
          ...
          logger.info(`Scheduler: booked queue entry ${entry.id}`);
        } else {
          await setQueueStatus(entry.id, "pending");
          logger.warn(`Scheduler: booking failed for entry ${entry.id}: ${result.error}`);
        }
      } catch (error) {
        await setQueueStatus(entry.id, "pending");
        logger.error(`Scheduler: error processing entry ${entry.id}`, { error });
      }
    }
  } finally {
    isProcessing = false;
...
export function startScheduler(bot: Bot): void {
  logger.info("Starting scheduler (5 min interval)");
  intervalId = setInterval(() => processQueue(bot), INTERVAL_MS);
}
